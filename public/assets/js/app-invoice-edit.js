...
$(function () {
 ...
 const preview = document.getElementById("btn-preview");
  ...
  function calculateLabor(option_id) {
   ...
   function showStatus() {
    var status = document.getElementById('job-order-status').value ;
      if(status == 1) {
        $("#job-order-status").addClass("bg-label-warning");
        $("#job-order-status").removeClass("bg-label-info");
      } else if (status == 2) {
        $("#job-order-status").addClass("bg-label-info");
        $("#job-order-status").removeClass("bg-label-warning");
